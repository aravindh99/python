...
class NeuTTSAir:
    ...
    def close(self) -> None:
        """
        Explicitly release external resources to avoid interpreter shutdown order issues.
        """
        # Close llama.cpp backend if present
        try:
            if hasattr(self, "backbone") and self._is_quantized_model:
        ...
        # Best-effort cleanup for codec/backbone torch models
        try:
            if hasattr(self, "codec"):
                # Move off GPU if any and drop references
                if hasattr(self.codec, "to"):
                    try:
                        self.codec.to("cpu")
                    except Exception:
                        pass
        ...
        try:
            if hasattr(self, "backbone") and not self._is_quantized_model:
                if hasattr(self.backbone, "to"):
                    try:
                        self.backbone.to(torch.device("cpu"))
                    except Exception:
                        pass
        except Exception:
            pass
